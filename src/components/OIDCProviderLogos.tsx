...
export const FacebookLogo = () => (
  ...
  >
    ...
    <path
      fill='#FFFFFF'
      d='M16.671 15.543l.532-3.47h-3.328v-2.25c0-.949.465-1.874 1.956-1.874h1.514V4.996s-1.374-.235-2.686-.235c-2.741 0-4.533 1.662-4.533 4.669v2.632H7.078v3.47h3.047v8.385a12.118 12.118 0 003.75 0v-8.385h2.796z'
    />
  </svg>
);
...
export function detectProvider(
  ...
  if (
    ...
    lowerIssuer.includes('login.microsoft.com')
  ) {
    return 'microsoft';
  }
  ...
  if (
    lowerIssuer.includes('facebook') ||
    lowerIssuer.includes('graph.facebook.com')
  ) {
    return 'facebook';
  }
  if (
    lowerIssuer.includes('wechat') ||
    lowerIssuer.includes('weixin.qq.com') ||
    lowerIssuer.includes('open.weixin.qq.com')
  ) {
    return 'wechat';
  }
  if (
    lowerIssuer.includes('apple') ||
    lowerIssuer.includes('appleid.apple.com')
  ) {
    return 'apple';
  }
  if (
...
export function getProviderButtonStyle(
...
export function getProviderButtonText(
  provider: ReturnType<typeof detectProvider>,
  customText?: string,
): string {
...
export const OIDCProviderLogo: React.FC<{
  provider: ReturnType<typeof detectProvider>;
}> = ({ provider }) => {
  switch (provider) {
    case 'google':
