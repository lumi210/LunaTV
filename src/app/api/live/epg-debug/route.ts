  ...
  try {
    ...
    if (!sourceKey) {
      return NextResponse.json(
        {
          error:
            '缺少 source 参数，用法: /api/live/epg-debug?source=你的直播源key',
        },
        { status: 400 },
      );
    }
      ...
      return NextResponse.json(
        {
          error: 'EPG URL 未配置',
          solution:
            '在后台管理页面的直播源配置中填写 EPG URL，或确保 M3U 文件中包含 x-tvg-url 或 url-tvg 参数',
        },
        { status: 400 },
      );
    }
    ...
    if (
      channelsWithEpg === 0 &&
      debug.totalEpgChannels > 0 &&
      debug.totalM3uChannelMappings > 0
    ) {
      issues.push({
        level: 'error',
        message: 'EPG 和 M3U 都有数据，但没有匹配到任何频道',
        solution:
          '频道名称可能差异太大，无法进行模糊匹配。查看下方的采样数据来对比 EPG 和 M3U 的频道名称。',
      });
    }

    const matchRate =
      totalChannels > 0
        ? ((channelsWithEpg / totalChannels) * 100).toFixed(1)
        : '0';
