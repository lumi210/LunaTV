...
export async function POST(req: NextRequest) {
  try {
    ...
    try {
      ...
      // 推广模式下的卡密处理标志
      let promotionCardKeyToBind: string | undefined = undefined;

      // 推广模式下，先生成卡密（但不绑定），等用户创建后再绑定
      if (cardKeyEnabled && systemMode === 'promotion' && !cardKey) {
        console.log('=== 推广模式：预生成卡密 ===');
        console.log('推广卡密类型:', promotionCardKeyType);

        const { cardKeyService } = await import('@/lib/cardkey');
        const plainKey = cardKeyService.generateRandomCardKey();
        const hashedKey = await cardKeyService.hashCardKey(plainKey);
        const now = Date.now();
        const expiresAt =
          cardKeyService.calculateExpiryDate(promotionCardKeyType);

        // 创建卡密记录（暂不绑定用户）
        const newCardKey: import('@/lib/types').CardKey = {
          key: plainKey,
          keyHash: hashedKey,
          keyType: promotionCardKeyType,
          status: 'unused', // 暂时标记为未使用
          createdAt: now,
          expiresAt: expiresAt,
          source: 'promotion_register',
        };

        await db.createCardKey(newCardKey);
        promotionCardKeyToBind = plainKey;
