  ...
  try {
    ...
    if (!html) {
      ...
      if (!response.ok) {
        ...
        if (
          ...
          try {
            ...
            throw new DoubanError(
              '豆瓣 API 和 Mobile API 均不可用，请稍后再试',
              'NETWORK_ERROR',
              response.status,
            );
          }
      ...
      if (isDoubanChallengePage(html)) {
        ...
        if (doubanCookies) {
          console.warn(
            `[Douban] ⚠️ 使用 Cookies 仍遇到 Challenge，Cookies 可能已失效`,
          );
        }
        ...
        if (enablePuppeteer) {
          ...
          try {
            ...
            try {
              ...
              throw new DoubanError(
                '豆瓣反爬虫激活，Puppeteer 和 Mobile API 均不可用',
                'RATE_LIMIT',
                429,
              );
            }
...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (error instanceof Error && error.message.includes('解析')) {
      ...
      const parseErrorSize = Buffer.byteLength(
